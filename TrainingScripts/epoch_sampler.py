...
login(token="ENTER HUGGING FACE TOKEN HERE")
...
def build_pools() -> Dict[str, pd.DataFrame]:
    ...
    def bespoke_conv(example):
        instr = example.get('instruction', example.get('prompt',''))
        resp = example.get('output', example.get('response',''))
        # Detect potential RAG responses by presence of context-like fields or markers
        rag_like = False
        for key in ("context", "retrieved", "sources", "passage", "evidence"):
            if example.get(key):
                rag_like = True
                break
        if not rag_like:
            txt = str(resp or '')
            if any(w in txt.lower() for w in ("context:", "source:", "references:", "retrieved:")):
                rag_like = True
        resp, _ = enforce_tag(resp or '', "<think>")
        if rag_like:
            resp = f"{resp}\n[RAG RESPONSE:] {str(example.get('answer', '') or '').strip()}".strip()
    ...
    def glaive_conv(example):
        ...
        # Normalize tool outputs for tokenization
        norm_msgs = []
        for m in msgs:
            role = m.get('role')
            content = str(m.get('content',''))
            if role in ('tool', 'function') or 'tool_response' in content.lower():
                content = f"[TOOL RESPONSE:] {content}"
            norm_msgs.append({"role": role, "content": content})
        last_assistant_idx = None
        for idx in range(len(norm_msgs)-1, -1, -1):
            if norm_msgs[idx].get('role') == 'assistant':
                last_assistant_idx = idx
                break
        if last_assistant_idx is None:
            target = ""
            context = [m for m in norm_msgs if m.get('role') != 'system']
        else:
            context = [m for i, m in enumerate(norm_msgs[:last_assistant_idx]) if m.get('role') != 'system']
            target = norm_msgs[last_assistant_idx].get('content', '')
